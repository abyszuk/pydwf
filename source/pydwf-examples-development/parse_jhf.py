...
def parse_jhf_file(filename: str):
    ...
    with open(filename, "r") as fi:
        ...
        for (index, line) in enumerate(fi):
            ...
            glyph = (x_left, x_right, tuple(strokes))
...
def make_jhf_image(filename):
    ...
    y_min = +math.inf
    y_max = -math.inf
    for (x_left, x_right, strokes) in characters:
        for stroke in strokes:
            for (x, y) in stroke:
                x_min = min(x_min, x)
                x_max = max(x_max, x)
    ...
    for (index, (x_left, x_right, strokes)) in enumerate(characters, 0):
        for stroke in strokes:
...
#for filename in glob.glob("*.jhf"):
#    print("Processing {} ...".format(filename))
#    make_jhf_image(filename)

if __name__ == "__main__":
    font = parse_jhf_file("futural.jhf")
    render_string(font, "Sidney is gaaf!", 3, "sidney.png")
